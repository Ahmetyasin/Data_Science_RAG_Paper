...
vectorstore = Chroma(persist_directory="../Vector_DB/chroma_grobid_fine_tuned", embedding_function=embeddings)
...
def run_experiment(run_number):
    ...
    for query in detailed_questions:
        ...
        contexts.append([doc.page_content for doc in retriever.get_relevant_documents(query,search_kwargs={'k' : 6})])
        print(answers[-1])
...
all_results.to_csv('./Results/Results_pipeline_grobid_fine_tuning.csv', index=False)
print("All runs completed and results saved to ./Results/Results_pipeline_grobid_fine_tuning.csv")
